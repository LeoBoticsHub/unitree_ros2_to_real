...
To simplify interaction with the services spawn by `unitree_high_ros2_control` node, we provided a library containing the class `ClientServicesHandler` which provide a easy to use interface with ros services. \
See [unitree_services_handler.hpp](ros2_unitree_legged_real/include/ros2_unitree_legged_real/unitree_services_handler.hpp) and [unitree_services_handler.cpp](ros2_unitree_legged_real/src/unitree_services_handler.cpp) to have an hint on how to use it.
