...
#include "ros2_unitree_legged_real/unitree_low_ros2_control.hpp"

UnitreeRos2LowController::UnitreeRos2LowController():
    Node("unitree_low_ros2_control")
{   
    // read ros parameters
...
void UnitreeRos2LowController::init_class()
{
    ...
    // * set joint states size
    joint_trajectory_msg_.name.resize(N_MOTORS);
    joint_trajectory_msg_.position.resize(N_MOTORS);
    joint_trajectory_msg_.velocity.resize(N_MOTORS);
    joint_trajectory_msg_.effort.resize(N_MOTORS);    
